...
def setup_slurm(backend: str, port: str) -> None:
    ...
    print(proc_id, ntasks, num_gpus, proc_id % num_gpus, node_list, addr, os.environ["MASTER_PORT"], os.system("nvidia-smi -L"))
...
def sync_string_across_gpus(keys: list[str], device, dim=0):
    ...
    keys_serialized_tensor = torch.frombuffer(keys_serialized, dtype=torch.uint8).to(device)
    keys_serialized_tensor = sync_tensor_across_gpus(keys_serialized_tensor, dim=0, cat=False)
    keys = [key for keys in keys_serialized_tensor for key in pickle.loads(bytes(keys.cpu().tolist())) ]
    return keys
