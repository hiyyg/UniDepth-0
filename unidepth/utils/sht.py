    ...
    return torch.stack(
        [   
            ...
            * (1.66666666666667 * z * (1.5 * z2 - 0.5) - 0.666666666666667 * z)
            ...
            0.00931882475114763 * y * (52.5 - 472.5 * z2) * (3.0 * x2 - y2),
            0.0913054625709205 * xy * (3.0 * z * (52.5 * z2 - 7.5) - 30.0 * z),
            0.241571547304372 * y * (2.25 * z * (2.33333333333333 * z * (1.5 - 7.5 * z2) + 4.0 * z) + 9.375 * z2 - 1.875),
            -1.24747010616985 * z * (1.5 * z2 - 0.5)
            + 1.6840846433293
            * z
            * (
            ...
            * (
            ...
            0.000368189725644507 * (-6.0 * x2 * y2 + x4 + y4) * (3.75 * z * (4.33333333333333 * z * (5197.5 * z2 - 472.5) - 3150.0 * z) - 14293.125 * z2 + 1299.375),
            5.10587282657803e-5
            * x
            * (5.0 * z * (5197.5 - 67567.5 * z2) + 41580.0 * z)
            * (-10.0 * x2 * y2 + x4 + 5.0 * y4),
...
class SphHarm(torch.nn.Module):
    ...
    def forward(self, points: torch.Tensor) -> torch.Tensor:
        ...
        vals = torch.cat([vals.repeat(1, theta.shape[0]), torch.arange(theta.shape[0], device=theta.device).unsqueeze(0).repeat_interleave(vals.shape[1], dim=1)], dim=0)
        ...
        harmonics = torch.complex(legendre_vals * torch.real(vandermonde), legendre_vals * torch.imag(vandermonde))
    ...
    def _gen_recurrence_mask(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        m_mat, l_mat = torch.meshgrid(torch.arange(0, self.l_max + 1, device=self.device, dtype=self.dtype), torch.arange(0, self.l_max + 1, device=self.device, dtype=self.dtype), indexing="ij")
    ...
    def _recursive(self, i: int, p_val: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        ...
        h = torch.einsum("ij,ijk->ijk", coeff_0, torch.einsum("ijk,k->ijk", torch.roll(p_val, shifts=1, dims=1), x)) - torch.einsum("ij,ijk->ijk", coeff_1, torch.roll(p_val, shifts=2, dims=1))
        p_val = p_val + h
        return p_val
    ...
    def _gen_associated_legendre(self, x: torch.Tensor) -> torch.Tensor:
        ...
        p_offdiag = torch.einsum("ij,ij->ij", torch.einsum("i,j->ij", self.f_b, x), p[(diag_indices[0], diag_indices[1])])  # p[torch.diag_indices(l_max)])
