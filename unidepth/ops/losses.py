...
def masked_mae(data: torch.Tensor, mask: torch.Tensor, dim: Tuple[int, ...]):
    ...
    mask = mask.float()
    mask_sum = torch.sum(mask, dim=dim, keepdim=True)
    mask_mean = torch.sum(data.abs() * mask, dim=dim, keepdim=True) / torch.clamp(mask_sum, min=1.0)
    return mask_mean
...
def masked_mse(data: torch.Tensor, mask: torch.Tensor, dim: Tuple[int, ...]):
    ...
    mask = mask.float()
    mask_sum = torch.sum(mask, dim=dim, keepdim=True)
    mask_mean = torch.sum((data ** 2) * mask, dim=dim, keepdim=True) / torch.clamp(mask_sum, min=1.0)
    return mask_mean
...
def masked_weighted_mean_var(data: torch.Tensor, mask: torch.Tensor, weights: torch.Tensor, dim: Tuple[int, ...]):
    if mask is None:
        return data.mean(dim=dim, keepdim=True), data.var(dim=dim, keepdim=True)
    mask = mask.float()
    mask_mean = torch.sum(data * mask * weights, dim=dim, keepdim=True) / torch.sum(mask * weights, dim=dim, keepdim=True).clamp(min=1.0)
    # V1**2 - V2, V1: sum w_i, V2: sum w_i**2
    denom = torch.sum(weights * mask, dim=dim, keepdim=True).square() - torch.sum((mask * weights).square(), dim=dim, keepdim=True)
    # correction is V1 / (V1**2 - V2), if w_i=1 => N/(N**2 - N) => 1/(N-1) (unbiased estimator of variance, cvd)
    correction_factor = torch.sum(mask * weights, dim=dim, keepdim=True) / denom.clamp(min=1.0)
    mask_var = correction_factor * torch.sum(weights * mask * (data - mask_mean) ** 2, dim=dim, keepdim=True) 
...
class SILog(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        if interpolate:
            input = F.interpolate(input, target.shape[-2:], mode='bilinear', align_corners=False)
        if mask is not None:
        ...
        if input.shape[1] > 1:
            input_ = torch.cat([input[:, :-1], self.input_fn(input[:, -1:].clamp(min=self.eps))], dim=1)
            target_ = torch.cat([target[:, :-1], self.input_fn(target[:, -1:].clamp(min=self.eps))], dim=1)
        ...
        if self.abs_rel:
            scale_error = (input - target).abs()[:, -1:] / target[:, -1:].clip(min=self.eps)
...
class MSE(nn.Module):
    ...
    def forward(
        ...
        batched_error = masked_mean(self.output_fn(mean_error.clamp(self.eps)), batch_mask, dim=(0,))
...
class SelfCons(nn.Module):
    ...
    def forward(
        ...
        rescales = input.shape[-2] / torch.tensor([x["resized_shape"][0] for x in metas], device=device)
        cams = torch.cat([x["K_target"] for x in metas], dim=0).to(device)
        flips = torch.tensor([x["flip"] for x in metas], device=device)
        
        iters = zip(input.chunk(chunks), mask.chunk(chunks), cams.chunk(chunks), rescales.chunk(chunks), flips.chunk(chunks))
        inputs0, inputs1, masks = [], [], []
        for i, (pair_input, pair_mask, pair_cam, pair_rescale, pair_flip) in enumerate(iters):
            ...
            # apply zoom
            input0 = F.interpolate(input0.unsqueeze(0), scale_factor=zoom_x, mode="bilinear", align_corners=True).squeeze(0)
            mask0 = F.interpolate(mask0.unsqueeze(0), scale_factor=zoom_x, mode="nearest").squeeze(0)
            ...
            input0 = F.pad(input0, (pad_left, pad_right, pad_top, pad_bottom), mode="constant", value=0)
            mask0 = F.pad(mask0, (pad_left, pad_right, pad_top, pad_bottom), mode="constant", value=0)
            input0 = input0[:, crop_top:input0.shape[-2]-crop_bottom, crop_left:input0.shape[-1]-crop_right]
            mask0 = mask0[:, crop_top:mask0.shape[-2]-crop_bottom, crop_left:mask0.shape[-1]-crop_right]
    ...
    def loss(
        ...
        loss = masked_mean((input - target).square().mean(dim=1), mask=mask, dim=(-2, -1))
