...
from unidepth.utils.checkpoint import checkpoint
from unidepth.utils import is_main_process, get_params
...
class ConvNeXtV2(nn.Module):
    ...
    def __init__(self,
        ...
        cur = 0
        for i in range(4):
            stage = nn.ModuleList([Block(dim=dims[i], drop_path=dp_rates[cur + j], use_checkpoint=use_checkpoint) for j in range(depths[i])])
