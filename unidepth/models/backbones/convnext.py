...
from timm.layers import trunc_normal_, AvgPool2dSame, DropPath, Mlp, GlobalResponseNormMlp, LayerNorm2d,\
    LayerNorm, create_conv2d, get_act_layer, make_divisible, to_ntuple
...
class ConvNeXt(nn.Module):
    def __init__(
        ...
        self.embed_dims = [int(dim) for i, dim in enumerate(dims) for _ in range(depths[i])]
    ...
    def forward(self, x, masks=None):
        ...
        if masks is not None:
            masks = torch.nn.functional.interpolate(masks.float(), size=x.shape[-2:], mode="nearest")
...
HF_URL = {
    "convnext_xxlarge_pt": ("laion/CLIP-convnext_xxlarge-laion2B-s34B-b82K-augreg-soup", "open_clip_pytorch_model.bin"),
    "convnext_large_pt": ("laion/CLIP-convnext_large_d_320.laion2B-s29B-b131K-ft-soup", "open_clip_pytorch_model.bin"),
    "convnext_large": ("timm/convnext_large_mlp.clip_laion2b_soup_ft_in12k_in1k_384", "pytorch_model.bin"),
}
