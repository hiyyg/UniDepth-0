...
class ListAdapter(nn.Module):
    ...
    def _forward(self, x: torch.Tensor, splits: torch.Tensor) -> torch.Tensor:
    ...
    def forward(
        ...
        return checkpoint(
            self._forward, (x, splits), self.parameters(), self.checkpoint
        )
...
class Decoder(nn.Module):
    ...
    def build(cls, config):
        obj = cls(
            ...
            checkpoint=config["training"]["use_checkpoint"],
