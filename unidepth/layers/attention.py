...
class AttentionBlock(nn.Module):
    ...
    def forward(
        ...
        context = x if context is None else context
        x = self.ls1(self.attn(x, rope=rope, attn_bias=attn_bias, context=context, pos_embed=pos_embed, pos_embed_context=pos_embed_context)) + x
...
class AttentionDecoderBlock(nn.Module):
    ...
    def forward(
        ...
        context = x if context is None else context
        x = self.ls1(self.cross_attn(x, rope=rope, attn_bias=attn_bias, context=context, pos_embed=pos_embed, pos_embed_context=pos_embed_context)) + x
